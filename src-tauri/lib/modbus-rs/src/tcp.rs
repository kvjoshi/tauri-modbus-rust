...
impl Transport {
    ...
    fn read(&mut self, fun: &Function) -> Result<Response> {
        ...
        let mut buff: Vec<u8> = header.pack()?;
            ...
            Ok(_s) => {
                ...
                match self.stream.read(&mut reply) {
                    Ok(_s) => {
                        ...
                        Ok(Response {
                            send_frame: buff,
                            resp_frame: reply.clone(),
                            data: Transport::get_reply_data(&reply, expected_bytes)?,
                        })
                    }
    ...
    fn write_single(&mut self, fun: &Function) -> Result<Response> {
        ...
        Ok(self.write(&mut buff)?)
    }

    fn write_read_multiple(&mut self, fun: &Function) -> Result<Response> {
    ...
    fn write_multiple(&mut self, fun: &Function) -> Result<Response> {
    ...
    fn write(&mut self, buff: &mut [u8]) -> Result<Response> {
        if buff.is_empty() {
            ...
            Ok(_s) => {
                ...
                match self.stream.read(reply) {
                    Ok(_s) => {
                        ...
                        Transport::validate_response_code(&buff, reply)?;
                        Ok(Response {
                            send_frame: buff.to_vec(),
                            resp_frame: reply.to_vec(),
                            data: Vec::new(),
                        })
                    }
                    Err(e) => Err(Error::Io(e)),
...
impl Client for Transport {
    ...
    fn read_coils(&mut self, addr: u16, count: u16) -> Result<Vec<Coil>> {
        let Response {
            ...
            resp_frame,
            data,
        } = self.read(&Function::ReadCoils(addr, count))?;
        Ok(binary::unpack_bits(&data, count))
    }
    ...
    fn read_discrete_inputs(&mut self, addr: u16, count: u16) -> Result<Vec<Coil>> {
        let Response {
            ...
            resp_frame,
            data,
        } = self.read(&Function::ReadDiscreteInputs(addr, count))?;
        Ok(binary::unpack_bits(&data, count))
    }

    /// Read `count` 16bit registers starting at address `addr`.
    fn read_holding_registers(&mut self, addr: u16, count: u16) -> Result<Output> {
        let Response {
            send_frame,
            resp_frame,
            data,
        } = self.read(&Function::ReadHoldingRegisters(addr, count))?;
        Ok(Output {
            send_frame: send_frame,
            resp_frame: resp_frame,
            value: binary::pack_bytes(&data[..])?,
        })
    }
    ...
    fn read_input_registers(&mut self, addr: u16, count: u16) -> Result<Vec<u16>> {
        let Response {
            ...
            resp_frame,
            data,
        } = self.read(&Function::ReadInputRegisters(addr, count))?;
        binary::pack_bytes(&data[..])
    }

    /// Write a single coil (bit) to address `addr`.
    fn write_single_coil(&mut self, addr: u16, value: Coil) -> Result<Output> {
        let Response {
            send_frame,
            resp_frame,
            data,
    ...
    fn write_single_register(&mut self, addr: u16, value: u16) -> Result<Output> {
        let Response {
            send_frame,
            resp_frame,
            data,
    ...
    fn write_multiple_coils(&mut self, addr: u16, values: &[Coil]) -> Result<Output> {
    ...
    fn write_multiple_registers(&mut self, addr: u16, values: &[u16]) -> Result<Output> {
    ...
    fn write_read_multiple_registers(
        ...
        let Response {
            ...
            resp_frame,
            data,
        } = self.write_read_multiple(&Function::WriteReadMultipleRegisters(
            write_address,
            write_quantity,
            &write_bytes,
            read_address,

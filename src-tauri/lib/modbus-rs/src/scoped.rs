            ...
            .map(|output| {
                if output.value.len() == 1 {
                    let drop_value = match self.fun {
                        ...
                        RegisterDropFunction::Increment => output.value[0] + 1,
                        RegisterDropFunction::Decrement => output.value[0] - 1,
                        RegisterDropFunction::Value(v) => v,
                        RegisterDropFunction::Fun(f) => f(output.value[0]),
                    };
                    let _ = self
                        .transport
                        .write_single_register(self.address, drop_value)
                        .expect("Failed to write single register during drop");
                }
            });
    }
}
